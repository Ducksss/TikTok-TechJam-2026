...
For TikTok-like creator operations, the policy priority is different from the
historical balanced V1 point: constrain false-positive rate first, then reduce
false negatives within that validated cap. Select any consequential threshold
on separate representative calibration data; do not infer it from the service
default or TEST1 labels.

## 8. Benchmark reproducibility boundary

This branch contains the inference release and public benchmark evidence, but
not private split rows, row-level TEST1 predictions, per-image protected
scores, datasets, cached features/logits, or checkpoints. Therefore a fresh
numerical rerun of V1/V2 or an end-to-end TEST1 replay is intentionally not
possible from `submission/` alone. The included artifacts support exact
integrity checks and aggregate metric inspection:
...
python3 -m json.tool \
  submission/evidence/test1/source-integrity.json >/dev/null
sed -n '1,8p' submission/evidence/test1/metrics_full.csv
sed -n '1,5p' submission/evidence/test1/robustness_deltas.csv
```

The TEST1 aggregate report is
[`evidence/test1/README.md`](evidence/test1/README.md). Its source-input hashes
record the received 30,000-row prediction file without publishing it. Fresh
verification reproduced all six confusion matrices, ROC-AUC values, and
grouped-tie average-precision values. The received package omitted its
referenced paired-bootstrap JSON, so confidence-interval endpoints were not
independently regenerated.

TEST1's corrected-v2 Expert-4/router and stored-head topology is not implemented
by the released `infer/` modules. Running the public four-expert CLI on the same
images would be a different experiment and must not be described as a TEST1
reproduction.

The Day 3 Professor Ng image and interview-only transcript are documentary
research inputs, not reproducible model evidence. Their identities are recorded
in [`../docs/INTERVIEW_PROF_NG.md`](../docs/INTERVIEW_PROF_NG.md); the brief
local-camera-statistics prototype has no final metric or released code path to
reproduce.
