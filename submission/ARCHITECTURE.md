...
2. A checkpoint preflight verifies the expected byte size and SHA-256 digest
   of all four expert state dictionaries before any checkpoint is
   deserialized.
...
The V1 balanced point is a historical research operating point, not the
recommended threshold for a TikTok-like consequential workflow. Creator
operations should constrain false-positive rate first, then reduce false
negatives within that cap, using separate representative calibration, slice
monitoring, human review, and appeals.
...
TEST1 is also outside this production diagram. It evaluates a benchmark-only
corrected-v2 Expert-4/router and stored-head topology, not the released
four-expert arithmetic mean shown here.

The camera-statistics direction discussed in the Day 3 Professor Ng interview
is outside this diagram as well. It received only a brief exploratory prototype,
was not strong or stable enough for a final result, and is not implemented in
