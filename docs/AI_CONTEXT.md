...
detector uses one frozen upstream FeatDistill Expert 4 SigLIP encoder/teacher
...
FeatDistill remains the credited UESTC research and checkpoint lineage.
SynthFlag did not originate or train Expert 4. The residual heads, routing,
...
- Teacher: upstream FeatDistill `Expert_4_siglip.pth`, a SigLIP So400M
...
- The repository began from a copied FeatDistill snapshot. Current source is an
  independently organized implementation, not a clean-room claim. Preserve
  `NOTICE`, attribution, and `docs/IMPLEMENTATION_PROVENANCE.md`.
