...
in live inference, or reproduce FeatDistill's paper-described training policy.
...
reproducibility model. FeatDistill attribution remains required for the detector
architecture and external checkpoints, but these augmentation utilities are
not presented as FeatDistill source or as a reconstruction of its training
library. The historical finding and mechanical overlap guard remain documented
in [`IMPLEMENTATION_PROVENANCE.md`](IMPLEMENTATION_PROVENANCE.md).
