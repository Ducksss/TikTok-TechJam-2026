...
def siglip_vision_config() -> SiglipVisionConfig:
    """Describe the frozen FeatDistill Expert 4 SigLIP encoder."""
...
def _materialize_teacher(checkpoint_path: Path, device: torch.device) -> SiglipTeacher:
    ...
    try:
        ...
        raise RuntimeError(
            f"checkpoint does not match FeatDistill Expert 4: {checkpoint_path}"
        ) from exc
    del state
...
FeatDistillEnsemble = SynthFlagDetector
