#!/usr/bin/env python3
"""Reject byte-identical FeatDistill source from the current SynthFlag tree."""
...
AUDIT_MANIFEST = ROOT / "docs/provenance/featdistill-upstream.json"
...
def find_identical_upstream_files() -> list[tuple[str, list[str]]]:
    ...
    matches = []
    for current_file in _candidate_files():
...
def main() -> int:
    ...
    print(f"- upstream: {manifest['source_repository']}")
    print(f"- audited commit: {manifest['source_commit']}")
    print(f"- current files scanned: {len(_candidate_files())}")
