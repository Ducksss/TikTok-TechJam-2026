...
This stateless FastAPI wrapper exposes the selected TEST1 Expert 4 plus
three-head routed detector to the SynthFlag `/try` experience. Image or derived-frame bytes are
...
queue. Run exactly one Uvicorn worker per GPU container so the large Expert 4 model
are not duplicated in device memory.

For a public demo, use a suitable GPU, mount Expert 4 and the three heads
privately, set `SYNTHFLAG_EAGER_LOAD=1`, and point both the web
