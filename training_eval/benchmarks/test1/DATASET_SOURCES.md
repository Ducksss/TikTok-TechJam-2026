...
The original CIFAKE sampling protocol is hash-pinned in `integrity.json`; source manifests containing local paths are intentionally not published.
...
The original SID sampling protocol is hash-pinned in `integrity.json`; source manifests containing local paths are intentionally not published.
...
DALL-E, Midjourney, and Stable Diffusion fake strata were excluded. Real COCO images were also excluded. This reduced overlap with earlier organizer demonstrations and documented upstream-detector source families, but it does not prove zero exposure through Expert 4 or SigLIP pretraining.

The original WildFake sampling protocol is hash-pinned in `integrity.json`; source manifests containing local paths are intentionally not published.
